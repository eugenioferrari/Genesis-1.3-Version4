...
void Control::applySlippage(double slippage, Field *field)
{
    ...
    while(abs(field->accuslip)>(sample*0.8)){
        ...
        // last was the last slice to be transmitted to the succeeding node and then filled with
        // the field from the preceding node, making it now the start of the field record.
        field->first=last;
        if (direction<0){
